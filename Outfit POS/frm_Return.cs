...
namespace Outfit_POS
{
    public partial class frm_Return : Form
    {
        ...
        public frm_Return(int _StaffID)
        {
            InitializeComponent();
            invoice = "";
            StaffID = _StaffID;
        ...
        private void ToolStripButton4_Click(object sender, EventArgs e)
        {
            ...
            AddSalesreturn();
            AddSalesreturnitem();
            frm_PrintReturn pr = new frm_PrintReturn(ListView1.FocusedItem.SubItems[1].Text);//label9.Text);
            pr.ShowDialog();
            this.Close();
        }
        ...
        {
            double quanty = 9999999999;
            double.TryParse(ListView2.FocusedItem.SubItems[2].Text, out quanty);
            if (quanty != 9999999999)
            {
                frm_Return_Q dev = new frm_Return_Q(ListView1.FocusedItem.SubItems[1].Text, quanty);

                if (dev.ShowDialog(this) == DialogResult.OK)
                {
                    double res = quanty - dev.ret;
                    string InvoiceNo, ProductNo,  returned;
                    double dItemPrice = 99999999;
                    double money, money_Back;
                    Double.TryParse(ListView2.FocusedItem.SubItems[3].Text, out dItemPrice);
                    money = dItemPrice * res ;
                    moneyBak += money;
                    Double.TryParse(ListView1.FocusedItem.SubItems[5].Text, out money_Back);
                  //  ListView1.FocusedItem.SubItems[5].Text= Strings.Format(money_Back+money, "#,##0.00");
                    InvoiceNo = ListView1.FocusedItem.SubItems[1].Text;
                    ProductNo = ListView2.FocusedItem.SubItems[0].Text;
                    //ItemPrice = ListView2.FocusedItem.SubItems[3].Text;
                    ListView2.FocusedItem.SubItems[2].Text=  Strings.Format(dev.ret, "#,##0.00");
                    ListView2.FocusedItem.SubItems[4].Text = Strings.Format(dItemPrice * dev.ret, "#,##0.00");
                    returned = Strings.Format(res, "#,##0.00");
                    ListView2.FocusedItem.SubItems[5].Text = Strings.Format(res, "#,##0.00");
                } 
            }
